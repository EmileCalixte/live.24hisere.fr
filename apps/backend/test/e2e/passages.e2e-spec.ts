...
describe.skip("Admin PassagesController (e2e)", () => {
  ...
  it("Get all passage list (GET /admin/passages)", async () => {
    const [withHiddenPassagesResponse, withoutHiddenPassagesResponse] = await Promise.all([
      request(app.getHttpServer()).get("/admin/passages").set("Authorization", ADMIN_USER_ACCESS_TOKEN),
    ...
    for (const response of [withHiddenPassagesResponse, withoutHiddenPassagesResponse]) {
      ...
      for (const passage of json.passages) {
        expect(passage).toContainAllKeys(["id", "detectionId", "importTime", "runnerId", "time", "isHidden"]);
