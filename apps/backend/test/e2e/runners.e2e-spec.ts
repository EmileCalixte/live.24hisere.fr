...
describe("Runner endpoints (e2e)", { concurrent: false }, () => {
    ...
    describe("Admin RunnersController (e2e)", async () => {
        ...
        it("Get a runner (GET /admin/runners/{id})", async () => {
            const [
                publicRunnerResponse,
                privateRunnerResponse,
                notFoundResponse,
                invalidIdResponse,
            ] = await Promise.all([
            ...
            expect(JSON.parse(notFoundResponse.text)).toEqual(
                notFoundBody(ERROR_MESSAGE_RUNNER_NOT_FOUND),
            );

            expect(JSON.parse(invalidIdResponse.text)).toEqual(
                badRequestBody(ERROR_MESSAGE_RUNNER_ID_MUST_BE_NUMBER),
            );
        });

        const runnersToPost: Array<
            Omit<Runner, "birthYear"> & { birthYear: number | string }
        > = [
            {
                id: 1234567,
        ...
        describe("Create a runner (POST /admin/runners)", async () => {
            ...
            it("Try to create a runner with already existing ID", async () => {
                ...
                expect(JSON.parse(response.text)).toEqual(
                    badRequestBody(ERROR_MESSAGE_RUNNER_ID_ALREADY_EXISTS),
                );
            });
        ...
        describe("Edit a runner (PATCH /admin/runners)", async () => {
            ...
            it("Test valid PATCH bodies", async () => {
                ...
                for (const { patchValues, expectedValues } of values) {
                    ...
                    expect(json.runner).toContainEntries(
            ...
            describe("Edit runner ID with passages", async () => {
                ...
                it("Edit ID and firstname", async () => {
                    ...
                    expect(runner.id).toBe(runnersToPost[0].id);
                    expect(runner.firstname).toBe(
                        "Edited with ID and passages",
                    );
                });
            });
        });
