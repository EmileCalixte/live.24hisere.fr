...
export class MiscService extends EntityService {
  ...
  private async getLine(key: string): Promise<Misc | null> {
    const miscs = await this.db.select().from(TABLE_MISC).where(eq(TABLE_MISC.key, key));
  ...
  private async saveLine(key: string, value: string): Promise<Misc> {
    await this.db.insert(TABLE_MISC).values({ key, value }).onDuplicateKeyUpdate({ set: { value } });

    const newLine = await this.getLine(key);

    if (!newLine) {
      throw new Error(`Failed to get updated misc data from database (key: ${key})`);
    }
