...
export class RunnerService extends EntityService {
  ...
  async getPublicRunnersOfRace(raceId: number): Promise<RunnerWithPassages[]> {
    ...
    const runnerPassages = await Promise.all(
      runners.map(async (runner) => await this.passageService.getPublicPassagesByRunnerId(runner.id)),
    );
  ...
  async createRunner(runnerData: Runner): Promise<Runner> {
    ...
    if (!newRunner) {
      throw new Error(`Failed to get created runner data in database (created runner ID: ${runnerData.id}`);
    }
  ...
  async updateRunner(runnerId: number, newRunnerData: Partial<Runner>): Promise<Runner> {
    ...
    return await this.db.transaction(async (tx) => {
      ...
      const dataToInsert = objectUtils.assignDefined(runner, newRunnerData);

      await tx.insert(TABLE_RUNNER).values(dataToInsert);

      const newRunner = this.getUniqueResult(
        await tx.select().from(TABLE_RUNNER).where(eq(TABLE_RUNNER.id, dataToInsert.id)),
      );

      if (!newRunner) {
        throw new Error(`Failed to get newly created runner data from database (new runner ID: ${dataToInsert.id})`);
      }

      // Then we change passage runnerIds
      await tx.update(TABLE_PASSAGE).set({ runnerId: newRunner.id }).where(eq(TABLE_PASSAGE.runnerId, runnerId));
  ...
  async deleteRunner(runnerId: number): Promise<boolean> {
    return await this.db.transaction(async (tx) => {
      await this.db.delete(TABLE_PASSAGE).where(eq(TABLE_PASSAGE.runnerId, runnerId));

      const [resultSetHeader] = await this.db.delete(TABLE_RUNNER).where(eq(TABLE_RUNNER.id, runnerId));
