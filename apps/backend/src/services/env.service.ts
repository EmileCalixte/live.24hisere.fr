...
type RequiredEnvironmentVariables = Record<(typeof REQUIRED_ENVIRONMENT_VARIABLES)[number], string>;

type OptionalEnvironmentVariables = {
  [K in keyof typeof OPTIONAL_ENVIRONMENT_VARIABLES]: string | (typeof OPTIONAL_ENVIRONMENT_VARIABLES)[K];
};

type EnvironmentVariables = RequiredEnvironmentVariables & OptionalEnvironmentVariables;
...
export class EnvService implements OnModuleInit {
  ...
  get<TName extends keyof EnvironmentVariables>(variableName: TName): EnvironmentVariables[TName] {
