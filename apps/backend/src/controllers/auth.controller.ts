...
import { ApiResponse, GetCurrentUserInfoApiRequest, LoginApiRequest } from "@live24hisere/core/types";
...
export class AuthController {
  ...
  @Post("/auth/login")
  async login(@Body() loginDto: LoginDto): Promise<ApiResponse<LoginApiRequest>> {
  ...
  async logout(@LoggedInUser() { accessToken }: AuthData): Promise<void> {
    await this.accessTokenService.deleteAccessTokenByStringToken(accessToken);
  }
