...
import { type CategoryShortCode, type PartialCategoriesDict, type PublicRace } from "@live24hisere/core/types";
...
export function useCategoryQueryString(
  ...
  const selectedCategory = React.useMemo<CategoryShortCode | null>(() => {
    ...
    return objectUtils.keys(categories).find((categoryCode) => categoryCode === searchParamsCategory) ?? null;
