...
export function useSpeedChartQueryString(): UseSpeedChartQueryString {
    ...
    const displayEachLapSpeed = React.useMemo(
        () => !searchParams.has(SearchParam.HIDE_LAP_SPEED),
        [searchParams],
    );
    const displayEachHourSpeed = React.useMemo(
        () => !searchParams.has(SearchParam.HIDE_HOUR_SPEED),
        [searchParams],
    );
    const displayAverageSpeed = React.useMemo(
        () => !searchParams.has(SearchParam.HIDE_AVG_SPEED),
        [searchParams],
    );
