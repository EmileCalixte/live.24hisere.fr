...
interface CustomSelectProps<T extends SelectOption["value"]>
    extends Omit<SelectProps<T>, "selectRef"> {
    searchable?: boolean;
...
export default function CustomSelect<T extends SelectOption["value"]>({
    ...
    const [shouldOpenOnSelectFocus, setShouldOpenOnSelectFocus] =
    ...
    const isOpened = React.useMemo<boolean>(() => {
        return (
            ((documentActiveElement === selectRef.current &&
                shouldOpenOnSelectFocus) ||
                openedContainerRef.current?.contains(documentActiveElement)) ??
            false
        );
    }, [documentActiveElement, shouldOpenOnSelectFocus]);

    const onSelectMouseDown: React.MouseEventHandler<HTMLSelectElement> = (
        e,
    ) => {
    ...
    const onSelectKeyDown: React.KeyboardEventHandler<HTMLSelectElement> = (
        e,
    ) => {
        if (e.key === Key.ESCAPE) {
            closeSelect();
    ...
    const onSearchInputKeyDown: React.KeyboardEventHandler<HTMLInputElement> = (
        e,
    ) => {
        switch (e.key) {
            case Key.ESCAPE:
    ...
    function updateSelectedOptionFromKeyboardArrow(
        key: Key.ARROW_UP | Key.ARROW_DOWN,
    ): void {
        if (!filteredOptions.length) {
            return;
        }
        ...
        if (
            ...
            (valueIndex === filteredOptions.length - 1 &&
                key === Key.ARROW_DOWN)
        ) {
            return;
        }
        ...
        selectOption(
            filteredOptions[valueIndex + (key === Key.ARROW_UP ? -1 : +1)]
                .value,
            false,
        );
    }
    ...
    React.useEffect(() => {
        ...
        optionsListRef.current.scrollTop =
            selectedButtonOffsetTop - optionsContainerHeight / 2;
            ...
            {isOpened && (
                ...
                >
                    {searchable && (
                        <Input
                            label={
                                searchInputLabelAndPlaceHolder ?? "Rechercher"
                            }
