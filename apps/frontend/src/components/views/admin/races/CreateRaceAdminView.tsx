...
export default function CreateRaceAdminView(): React.ReactElement {
    ...
    const [existingRaces, setExistingRaces] = useState<AdminRace[] | false>(
        false,
    );
    ...
    const existingRacesOptions = useMemo(
        () => getRacesSelectOptions(existingRaces),
        [existingRaces],
    );
    ...
    const onSelectRaceToCopy = useCallback(
        (e: React.ChangeEvent<HTMLSelectElement>) => {
            ...
            const raceToCopy = existingRaces.find(
                (race) => race.id.toString() === e.target.value,
            );
    ...
    const fetchExistingRaces = useCallback(async () => {
        ...
        if (!isApiRequestResultOk(result)) {
            ToastService.getToastr().error(
                "Impossible de récupérer la liste des courses existantes",
            );
            return;
        }
