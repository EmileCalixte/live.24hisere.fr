...
type RunnerSortedProcessedPassages = Record<
    number,
    Array<PublicProcessedPassage<AdminPassageWithRunnerId>>
>;
...
export default function FastestLapsAdminView(): React.ReactElement {
    ...
    const [passages, setPassages] = React.useState<
        AdminPassageWithRunnerId[] | false
    >(false);
    ...
    const [
        ...
        setDisplayOnlyOneFastestLapPerRunner,
    ] = React.useState(false);
    const [selectedRaceId, setSelectedRaceId] = React.useState<number | "ALL">(
        "ALL",
    );
    ...
    const fetchPassages = React.useCallback(async () => {
        ...
        if (!isApiRequestResultOk(result)) {
            ToastService.getToastr().error(
                "Impossible de récupérer la liste des passages",
            );
            return;
        }
    ...
    const runnerSortedPassages = React.useMemo<
        RunnerSortedPassages | false
    >(() => {
        if (!passages) {
            return false;
        }
    ...
    const runnerSortedProcessedPassages = React.useMemo<
        RunnerSortedProcessedPassages | false
    >(() => {
        ...
        for (const runnerId in runnerSortedPassages) {
            ...
            if (!runner) {
                console.warn(
                    `Runner ${runnerId} not found in runners array, ignoring its passages`,
                    runners,
                );
                continue;
            }
            ...
            sortedProcessedPassages[runnerId] =
                getProcessedPassagesFromPassages(race, runnerPassages);
        }
    ...
    const speedSortedProcessedPassages = React.useMemo<
        Array<PublicProcessedPassage<AdminPassageWithRunnerId>> | false
    >(() => {
        ...
        const sortedProcessedPassages: Array<
            PublicProcessedPassage<AdminPassageWithRunnerId>
        > = [];

        for (const runnerId in runnerSortedProcessedPassages) {
            const runnerProcessedPassages =
                runnerSortedProcessedPassages[runnerId];
        ...
        return sortedProcessedPassages
            ...
            .sort((p1, p2) => {
                ...
                if (
                    ...
                    p2.processed.lapStartRaceTime >=
                        p1.processed.lapStartRaceTime
                ) {
                    return 1;
                }
        ...
        if (selectedRaceId !== "ALL" && runners) {
            passages = passages.filter((passage) => {
                const runner = runners.find(
                    (runner) => runner.id === passage.runnerId,
                );
